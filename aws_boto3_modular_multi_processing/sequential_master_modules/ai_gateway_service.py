...
RHEL_RULES = (
    ...
    # ============================================================
    # RHEL (YUM) DOMAIN RULES — Applies ONLY when os_name = "RHEL" and os_version starts with "9"
    # Canonical, Ubuntu/Debian-aligned refactor (Revision 9)
    # ============================================================
    "These rules apply ONLY when os_name == 'RHEL' AND os_version starts with '9'. They MUST NOT apply to CentOS 7, CentOS 8, Amazon Linux, Fedora, Ubuntu, Debian, Alpine, BusyBox, macOS, Windows, PAN-OS, or Cisco IOS.\n"
    "IMPORTANT:\n"
    ...
    ##### Wrong package manager in pipelines (&&) — Patch2 cluster (RHEL canonical) #####
    "- If ALL segments in the pipeline are already valid for this OS, and NO segment uses\n"
    "  a package manager that does NOT belong to this OS, and the command succeeded\n"
    "  (exit_status = 0) with no stderr, the LLM MUST return 'fallback'.\n"
    "\n"
    "- When evaluating pipelines under this Patch2 rule, the presence of a system-wide\n"
    "  operation that is already valid for this OS (e.g., 'yum update -y') MUST NOT trigger\n"
    "  the OS-Mutation Guard. Such system-wide segments MUST be preserved verbatim and\n"
    "  MUST NOT cause fallback.\n"
    "\n"
    "- This previous exception for valid system-wide operations applies ONLY when the\n"
    "  pipeline contains at least one segment that uses a package manager that does\n"
    "  NOT belong to this OS. If NO such wrong-OS package-manager segment exists, the\n"
    ...
    "\n"
    "- If the command is a pipeline using '&&' and includes a package manager that does NOT belong to this OS\n"
    "  (for example: apt/apt-get, dnf, apk, pacman on RHEL; yum on Ubuntu/Debian; etc.),\n"
    "  the LLM MUST treat each segment independently.\n"
    "\n"
    "- If ALL segments in the pipeline are either:\n"
    "      • simple package-install commands, OR\n"
    ...
    "  It MUST return a FULL rewritten pipeline where:\n"
    "      • ONLY the wrong-OS package-manager install segments are rewritten using the correct package manager\n"
    "        for this OS (i.e., 'yum install -y <pkg>' on RHEL),\n"
    "      • ALL 'yum install' commands in the rewritten pipeline MUST include the '-y' flag to ensure non-interactive behavior, even if the original segment used 'yum' and did not require a package-manager rewrite,\n"
    "      • ALL other segments are preserved verbatim,\n"
    "      • The LLM MUST NOT drop, duplicate, reorder, or invent segments.\n"
    "\n"
    ...
    "  because it is system-wide.\n"
    "\n"
    "- For pipelines that contain a valid native 'yum' system-wide operation (e.g., 'yum update -y')\n"
    "  and one or more wrong-OS package-manager segments, these Patch2 rules OVERRIDE any earlier\n"
    "  malformed-command fallback guidance. The presence of multiple package managers in such a\n"
    "  pipeline MUST NOT be treated as ambiguous intent; the native 'yum' system-wide operation\n"
    "  MUST be treated as the anchor, and all wrong-OS segments MUST be rewritten to\n"
    "  'yum install -y <pkg>'.\n"
    "\n"
    "- If ANY segment contains an invalid or unsupported flag (see invalid-flag rules),\n"
    "  the LLM MUST use 'fallback'.\n"
    ...
    ##### Single-segment rewrite and fallback (RHEL) #####
    "- If the command is missing arguments (e.g., 'yum install'),\n"
    "  treat it as malformed and use 'fallback' unless a safe, concrete correction\n"
    "  can be constructed WITHOUT guessing a package name.\n"
    "- If the command is a SINGLE-SEGMENT command (no '&&') and uses a package manager\n"
    "  that does NOT match RHEL (apt, apt-get, apt-cache, dnf, apk, pacman, brew),\n"
...
# This is the refactored CentOS7 domain primitives block with the refactored patch2 rewrite ports, to be tested on gpt-5.6-Sol
CENTOS7_RULES = (

    # ============================================================
    # CentOS 7 (YUM) DOMAIN RULES — Applies ONLY when os_name = "CentOS" and os_version starts with "7"
    # Canonical, Ubuntu/Debian/RHEL‑aligned refactor (GPT‑5.6‑Sol)
    # ============================================================
    "These rules apply ONLY when os_name == 'CentOS' AND os_version starts with '7'. "
    "They MUST NOT apply to RHEL, CentOS 8, Amazon Linux, Fedora, Ubuntu, Debian, Alpine, BusyBox, macOS, Windows, PAN-OS, or Cisco IOS.\n"
    "IMPORTANT:\n"
    "- The \"instance_id\" and \"ip\" fields MUST NOT be used to determine the action or influence your decision.\n"
    "- The \"instance_id\" and \"ip\" fields MUST be ignored.\n"
    "\n"
    ...
    ##### CentOS 7 YUM domain primitives (canonical) #####
    "CentOS 7 YUM domain primitives:\n"
    ...
    "- If the command is destructive (e.g., 'rm -rf /'), the LLM MUST return 'abort' with a clear message.\n"
    "- If the command is unrecognized (exit_status 127) and not obviously a shell primitive, the LLM MUST use 'fallback'.\n"
    "\n"

    ##### Package Manager Classification (CentOS 7) #####
    "- The LLM MUST treat the following commands as package-manager install commands\n"
    "  when a concrete package name <pkg> is present:\n"
    "\n"
    "      * 'apt-get install <pkg>'\n"
    ...
    "      * 'brew install <pkg>'\n"
    "\n"
    "- For CentOS 7, the ONLY native package manager is:\n"
    "      * yum\n"
    "\n"
    "- ALL other package managers listed above MUST be treated as wrong-OS\n"
    "  package managers on CentOS 7.\n"
    "\n"
    "- When a wrong-OS package-manager install command appears in a segment and a\n"
    "  concrete package name is present, the LLM MUST treat that segment as a\n"
    "  wrong-OS package-manager install. In any rule that calls for rewriting\n"
    "  wrong-OS package-manager installs for CentOS 7, that segment MUST be rewritten\n"
    "  to use 'yum install -y <pkg>'.\n"
    "\n"

    ##### Wrong package manager in pipelines (&&) — Patch2 cluster (CentOS 7 canonical) #####
    "- If ALL segments in the pipeline are already valid for this OS, and NO segment uses\n"
    "  a package manager that does NOT belong to this OS, and the command succeeded\n"
    "  (exit_status = 0) with no stderr, the LLM MUST return 'fallback'.\n"
    "\n"
    ...
    "  MUST NOT cause fallback.\n"
    "\n"
    "- This exception for valid system-wide operations applies ONLY when the\n"
    "  pipeline contains at least one segment that uses a package manager that does\n"
    "  NOT belong to this OS. If NO such wrong-OS package-manager segment exists, the\n"
    "  LLM MUST apply the 'successful pipeline → fallback' rule instead.\n"
    "\n"
    "- If the command is a pipeline using '&&' and includes a package manager that does NOT belong to this OS\n"
    "  (for example: apt/apt-get, dnf, apk, pacman, brew on CentOS 7),\n"
    "  the LLM MUST treat each segment independently.\n"
    "\n"
    "- If ALL segments in the pipeline are either:\n"
    "      • simple package-install commands, OR\n"
    ...
    "  It MUST return a FULL rewritten pipeline where:\n"
    "      • ONLY the wrong-OS package-manager install segments are rewritten using the correct package manager\n"
    "        for this OS (i.e., 'yum install -y <pkg>' on CentOS 7),\n"
    "      • ALL 'yum install' commands in the rewritten pipeline MUST include the '-y' flag to ensure non-interactive behavior,\n"
    "      • ALL other segments are preserved verbatim,\n"
    "      • The LLM MUST NOT drop, duplicate, reorder, or invent segments.\n"
    "\n"
    "- The following commands are considered system-wide operations:\n"
    ...
    ##### Single-segment rewrite and fallback (CentOS 7) #####
    "- If the command is missing arguments (e.g., 'yum install'),\n"
    "  treat it as malformed and use 'fallback' unless a safe, concrete correction\n"
    "  can be constructed WITHOUT guessing a package name.\n"
    "- If the command is a SINGLE-SEGMENT command (no '&&') and uses a package manager\n"
    "  that does NOT match CentOS 7 (apt, apt-get, apt-cache, dnf, apk, pacman, brew),\n"
    "  the LLM MUST rewrite the command using 'yum' when a safe, concrete package name\n"
    "  is present. This includes commands such as 'apt install <pkg>' and\n"
    "  'apt-get install <pkg>', which MUST be rewritten to:\n"
    "      yum install -y <pkg>\n"
    ...
    ##### Idempotency and <pkg> binding (CentOS 7) #####
...
CENTOS8_RULES = (
    ...
    # ============================================================
    # CentOS 8 (DNF/YUM) DOMAIN RULES — Applies ONLY when os_name = "CentOS" and os_version starts with "8"
    # Canonical, RHEL‑aligned refactor (GPT‑5.6‑Sol)
    # ============================================================
    "These rules apply ONLY when os_name == 'CentOS' AND os_version starts with '8'. "
    "They MUST NOT apply to RHEL, CentOS 7, Amazon Linux, Fedora, Ubuntu, Debian, Alpine, BusyBox, macOS, Windows, PAN-OS, or Cisco IOS.\n"
    "IMPORTANT:\n"
    "- The \"instance_id\" and \"ip\" fields MUST NOT be used to determine the action or influence your decision.\n"
    "- The \"instance_id\" and \"ip\" fields MUST be ignored.\n"
    "\n"
    ...
    ##### CentOS 8 DNF/YUM domain primitives (canonical) #####
    "CentOS 8 DNF domain primitives:\n"
    ...
    ##### Package Manager Classification (CentOS 8) #####
    "- The LLM MUST treat the following commands as package-manager install commands\n"
    "  when a concrete package name <pkg> is present:\n"
    "\n"
    "      * 'apt-get install <pkg>'\n"
    ...
    "      * 'brew install <pkg>'\n"
    "\n"
    "- For CentOS 8, the native package manager is:\n"
    "      * dnf\n"
    "- 'yum' is a compatibility wrapper and MAY be normalized to 'dnf' when appropriate.\n"
    "\n"
    "- ALL other package managers listed above (apt, apt-get, apk, pacman, zypper, brew) MUST be treated as wrong-OS\n"
    "  package managers on CentOS 8.\n"
    "\n"
    "- When a wrong-OS package-manager install command appears in a segment and a\n"
    "  concrete package name is present, the LLM MUST treat that segment as a\n"
    "  wrong-OS package-manager install. In any rule that calls for rewriting\n"
    "  wrong-OS package-manager installs for CentOS 8, that segment MUST be rewritten\n"
    "  to use 'dnf install -y <pkg>'.\n"
    "\n"

    ##### Wrong package manager in pipelines (&&) — Patch2 cluster (CentOS 8 canonical) #####
    "- If ALL segments in the pipeline are already valid for this OS, and NO segment uses\n"
    "  a package manager that does NOT belong to this OS, and the command succeeded\n"
    "  (exit_status = 0) with no stderr, the LLM MUST return 'fallback'.\n"
    "\n"
    "- When evaluating pipelines under this Patch2 rule, the presence of a system-wide\n"
    "  operation that is already valid for this OS (e.g., 'dnf upgrade -y') MUST NOT trigger\n"
    "  the OS-Mutation Guard. Such system-wide segments MUST be preserved verbatim and\n"
    "  MUST NOT cause fallback.\n"
    "\n"
    "- This exception for valid system-wide operations applies ONLY when the\n"
    ...
    "\n"
    "- If the command is a pipeline using '&&' and includes a package manager that does NOT belong to this OS\n"
    "  (for example: apt/apt-get, apk, pacman, brew on CentOS 8),\n"
    "  the LLM MUST treat each segment independently.\n"
    "\n"
    "- If ALL segments in the pipeline are either:\n"
    "      • simple package-install commands, OR\n"
    ...
    "  It MUST return a FULL rewritten pipeline where:\n"
    "      • ONLY the wrong-OS package-manager install segments are rewritten using the correct package manager\n"
    "        for this OS (i.e., 'dnf install -y <pkg>' on CentOS 8),\n"
    "      • ALL 'dnf install' commands in the rewritten pipeline MUST include the '-y' flag to ensure non-interactive behavior,\n"
    "      • ALL other segments are preserved verbatim,\n"
    "      • The LLM MUST NOT drop, duplicate, reorder, or invent segments.\n"
    "\n"
    "- The following commands are considered system-wide operations:\n"
    ...
    ##### Single-segment rewrite and fallback (CentOS 8) #####
    ...
    "\n"
    "- If the command is a SINGLE-SEGMENT command (no '&&') and uses a package manager\n"
    "  that does NOT match CentOS 8 (apt, apt-get, apt-cache, apk, pacman, brew),\n"
    "  the LLM MUST rewrite the command using 'dnf' when a safe, concrete package name\n"
    "  is present. This includes commands such as 'apt install <pkg>' and\n"
    "  'apt-get install <pkg>', which MUST be rewritten to:\n"
    "      dnf install -y <pkg>\n"
    "\n"
    ...
    ##### OS‑signaled remediation (CentOS 8, DNF/YUM) #####
    ...
    ##### Nonexistent package on CentOS 8 (ambiguous, NOT remediation) #####
    "- If 'dnf install -y <pkg>' fails with wording similar to:\n"
    "    * 'No match for argument: <pkg>'\n"
    "    * 'Unable to find a match: <pkg>'\n"
    "  and stderr does NOT contain any DNF/YUM metadata, mirrorlist, repo corruption,\n"
    "  or rpmdb errors, the LLM MUST treat this as an ambiguous condition.\n"
    "  The LLM MUST return 'fallback' and MUST NOT propose 'dnf upgrade -y', 'dnf update -y', or any\n"
    "  other system-wide operation in cleanup or retry.\n"
    "\n"

    ##### Idempotency and <pkg> binding (CentOS 8) #####
    "- If stderr indicates the package is already installed, such as:\n"
...
ALPINE_RULES = (
    ...
    # ============================================================
    # ALPINE (APK) DOMAIN RULES — Applies ONLY when os_name = "Alpine"
    # Canonical, GLOBAL_RULES‑aligned refactor (Revision 14)
    # ============================================================
    ...
    "- If a malformed command references ANY package manager (apt, apt-get, yum, dnf, apk,\n"
    "  brew, pacman, zypper) and this OS block does NOT define a deterministic rewrite for\n"
    "  that manager, the LLM MUST use 'fallback'. This OS block MUST NOT guess which package\n"
    "  manager is correct.\n"
    "\n"
    "- This Linux-family OS MUST NOT introduce 'sudo' as part of malformed-command\n"
    "  correction. If a command fails due to permission issues and no deterministic\n"
    ...
    "  structure.\n"
    "- If stderr indicates a syntax error involving a pipeline or subshell (e.g.,\n"
    ...
    ##### Invalid package‑manager flags (Linux-family OSes) #####
    "- If an 'apk', 'apt', 'apt-get', 'yum', 'dnf', or 'pacman' command contains any unknown\n"
    "  or unsupported flags (for example: 'invalid option', 'unknown option', or flags not\n"
    "  documented for that package manager), the LLM MUST use 'fallback'.\n"
    "- The LLM MUST NOT attempt to correct, remove, rewrite, or guess the intended flag.\n"
    "- The LLM MUST NOT infer user intent for unknown flags.\n"
    "\n"

    # BusyBox addendum
    "These rules also apply when BusyBox applets are invoked on Linux-family OSes. BusyBox\n"
    "installed on a Linux distribution does NOT activate the BusyBox domain primitives block.\n"
    "\n"
    ...
    "  use 'fallback' unless a safe, concrete correction can be constructed WITHOUT guessing\n"
    "  a package name.\n"
    "\n"
    "- If the command uses a package manager that does not match Alpine (apt, apt-get, yum,\n"
    "  dnf, brew, pacman, zypper), the LLM MUST rewrite the command using 'apk add <pkg>'\n"
    ...
    "      * 'brew install <pkg>'\n"
    "\n"
    "- For Alpine, the ONLY native package manager is:\n"
    "      * apk\n"
    "\n"
    "- ALL other package managers listed above MUST be treated as wrong-OS package managers\n"
    "  on Alpine.\n"
    "\n"
    "- When a wrong-OS package-manager install command appears in a segment and a concrete\n"
    "  package name is present, the LLM MUST treat that segment as a wrong-OS package-manager\n"
    "  install. In any rule that calls for rewriting wrong-OS package-manager installs for\n"
    "  Alpine, that segment MUST be rewritten to use 'apk add <pkg>'.\n"
    "\n"
    ...
    "\n"
    "- This previous exception for valid system-wide operations applies ONLY when the\n"
    "  pipeline contains at least one segment that uses a package manager that does NOT\n"
    "  belong to this OS. If NO such wrong-OS package-manager segment exists, the LLM MUST\n"
    "  apply the 'successful pipeline → fallback' rule instead.\n"
    "\n"
    "- If the command is a pipeline using '&&' and includes a package manager that does NOT\n"
    "  belong to this OS (for example: yum, dnf, pacman, apt/apt-get on Alpine; apk on\n"
    "  Ubuntu/Debian/RHEL/CentOS/Fedora), the LLM MUST treat each segment independently.\n"
    "\n"
    "- If ALL segments in the pipeline are either:\n"
    "      • simple package-install commands, OR\n"
    "      • non-mutating, non–package-manager commands that are safe to preserve verbatim,\n"
    "      • system-wide operations that are already valid for this OS and do NOT require\n"
    "        rewriting,\n"
    "  AND at least one segment uses a wrong-OS package manager,\n"
    "  the LLM MUST use 'retry_with_modified_command' and MUST NOT use 'fallback'. It MUST\n"
    "  return a FULL rewritten pipeline where:\n"
    "      • ONLY the wrong-OS package-manager install segments are rewritten using the\n"
    "        correct package manager for this OS ('apk add <pkg>' on Alpine),\n"
    "      • ALL other segments are preserved verbatim,\n"
    "      • The LLM MUST NOT drop, duplicate, reorder, or invent segments.\n"
    "\n"
    ...
    "      zypper update\n"
    "\n"
    "- If ANY segment in the pipeline is a system-wide operation AND that segment would\n"
    "  require rewriting for this OS, the LLM MUST use 'fallback'.\n"
    "- If a system-wide segment is already valid for this OS and does NOT require rewriting,\n"
    "  the LLM MUST preserve it verbatim and MUST NOT fallback solely because it is\n"
    "  system-wide.\n"
    "\n"
    "- If ANY segment contains an invalid or unsupported flag (see invalid-flag rules),\n"
    "  the LLM MUST use 'fallback'.\n"
    ...
    ##### Single-segment rewrite and fallback (Alpine) #####
    "- If the command is missing arguments (e.g., 'apk add') and stderr indicates that a\n"
    "  package list is required, the LLM MUST use 'fallback' unless a safe correction exists\n"
    "  WITHOUT guessing a package name.\n"
    "\n"
    "- If a SINGLE-SEGMENT command uses ANY wrong-OS package manager install pattern:\n"
    "      apt-get install <pkg>\n"
    "      apt install <pkg>\n"
    ...
    ##### Nonexistent / unsatisfiable package (ambiguous, NOT remediation) #####
    "- If 'apk add <pkg>' fails with wording such as:\n"
    "      'ERROR: unable to select packages:'\n"
    "      'ERROR: unsatisfiable constraints:'\n"
    "  and stderr does NOT contain cache/index corruption or deterministic remediation\n"
    "  hints, the LLM MUST return 'fallback'.\n"
    "- The LLM MUST NOT propose 'apk update' or any other system-wide operation solely to\n"
    "  resolve unsatisfiable constraints.\n"
    "\n"

    ##### Alpine cache/index corruption (OS‑signaled remediation) #####
    ...
    "  indicate a network failure), the LLM MUST use 'cleanup_and_retry' with Alpine-specific\n"
    "  cleanup semantics:\n"
    ...
    ##### Revision 13.1 — history‑aware remediation #####
    "- If stderr contains 'unable to select packages:' AND the history contains a prior\n"
    "  'apk update', the LLM MUST use 'cleanup_and_retry' instead of 'fallback'.\n"
    ...
    ##### Network failures (Alpine, aligned with GLOBAL_RULES) #####
    "- If stderr indicates DNS or connectivity failures ('Could not resolve host',\n"
    "  'Connection timed out', 'No route to host', 'Temporary failure resolving',\n"
    "  'Network unreachable', 'Host unreachable'), the LLM MUST use 'fallback'.\n"
    "- These conditions indicate connectivity problems, NOT package corruption.\n"
    ...
    ##### Idempotency and <pkg> binding (Alpine) #####
    "- If stderr indicates idempotency (e.g., 'Nothing to do', 'Package <pkg> is already\n"
    "  installed', 'No packages marked for installation'), the LLM MUST use\n"
    "  'cleanup_and_retry' in accordance with global Idempotency rules. 'fallback' MUST NOT\n"
    "  be used for idempotency conditions.\n"
    "\n"
    "- For any rule referencing '<pkg>', the LLM MUST replace '<pkg>' with the package name\n"
    "  used in the failing command.\n"
    "- If the failing command does NOT include a package name (e.g., 'apk update'), the LLM\n"
    "  MUST NOT invent a package name and MUST omit any install step.\n"
    "\n"

    ##### Destructive and unrecognized commands #####
    "- If the command is destructive (e.g., 'rm -rf /'), the LLM MUST return 'abort'.\n"
    "- If the command is unrecognized (exit_status 127) and not obviously an Alpine/Unix\n"
    "  primitive, the LLM MUST use 'fallback'.\n"
    "\n"

    ##### sudo prohibition (Alpine) #####
    "- Alpine environments in this contract do NOT use 'sudo'. The LLM MUST NOT generate\n"
    "  'sudo' under any circumstances, including permission errors.\n"
    "\n"
    ...
    try:
        ...
        payload_old = {
            ...
            "input": (
                ...
                ## ============================================================
                ## AMAZON LINUX (YUM) DOMAIN RULES — Applies ONLY when os_name = "Amazon Linux" (Amazon Linux 2)
        ...
        ##### NOTE This is not the validator for the stress_tester.py. This validator here is an outgoing API response validator, 
        ##### whereas teh validator for the stress_tester.py is a validator for the LLM response for a given context based schema
        ##### test that is used to test the contract rules for robustness and determinism.
...
#### CONTRACT SPECIFIC NOTES FOR THE DOMAIN PRIITIVES BLOCKS ####
### **About the Contract Rules in `ai_gateway_service.py`**

#The full LLM contract is defined inside `ai_gateway_service.py` and includes detailed domain‑primitive blocks for every supported OS and platform. These blocks describe the exact semantics, safety rules, and recovery behaviors the LLM must follow when interpreting shell commands, handling package‑manager failures, or performing remediation. The file also contains extensive commentary explaining the differences between operating systems (e.g., Debian vs. Ubuntu APT behavior, Fedora vs. RHEL DNF semantics, Alpine APK quirks, BusyBox limitations). These comments are intended for developers maintaining the contract and are not part of the runtime logic. The README provides a high‑level overview, but the authoritative source of truth for all contract rules is the domain‑primitive section inside `ai_gateway_service.py`.


#### Here are the comment notes that have been stripped and removed from the original payload above to save on prefill tokeninzation
#### costs during testing. The comments reduce determinism from the LLM and should not be included in the payload.
#### Some additional notes to developers:
...
#### DOMAIN PRIMITIVES SPECIFIC COMMENT NOTES: 
