...
def validate_centos8_semantics(context: Dict[str, Any], resp: Dict[str, Any]) -> List[str]:
    """
    CentOS 8–specific semantic checks derived from the CentOS 8 DNF/YUM domain primitives
    (Revision 11) and global Linux malformed-command rules (Revision 6.8).

    Applies ONLY when os_name == "CentOS" AND os_version starts with "8".
    """
    ...
    # ------------------------------------------------------------
    # 1) Wrong package manager in RETRY on CentOS 8
    # ------------------------------------------------------------
    def _check_retry_wrong_pm(r: Any) -> None:
        if isinstance(r, str):
            if cmd_uses_wrong_package_manager(r, os_name):
                errors.append(
                    "On CentOS 8, retry must not use non-DNF package managers (apt/yum/apk/brew/etc.)."
                )
    ...
    if cmd_uses_wrong_package_manager(command, os_name) and pkg_from_cmd:
        if action != "retry_with_modified_command":
            errors.append(
                "On CentOS 8, when the original command uses a non-DNF package manager "
                "with a clear package name, the LLM must use 'retry_with_modified_command'."
            )
        else:
            if not isinstance(retry, str):
                errors.append(
                    "CentOS 8 wrong-package-manager rewrites must use a single string retry command."
                )
            else:
                expected_sub = f"dnf install -y {pkg_from_cmd}"
                if expected_sub not in retry:
                    errors.append(
                        f"CentOS 8 wrong-package-manager rewrite must include '{expected_sub}' "
    ...
    if "|" in command or "$(" in command or ")" in command:
        if (
            ...
            if action != "fallback":
                errors.append("Malformed pipeline/subshell on CentOS 8 must result in 'fallback'.")
    ...
    if looks_like_network_failure(stderr):
        if action != "fallback":
            errors.append("Network failures on CentOS 8 must use 'fallback' action.")
    ...
    if ("dnf install" in command or "yum install" in command) and "Need to pass a list of packages" in stderr:
        if action != "fallback":
            errors.append(
                "CentOS 8 malformed install (missing package name) must use 'fallback'."
            )

    # ------------------------------------------------------------
    # 6) DNF/YUM metadata or mirrorlist failures → cleanup_and_retry
    # ------------------------------------------------------------
    metadata_indicators = [
    ...
    if any(ind in stderr for ind in metadata_indicators):
        if action != "cleanup_and_retry":
            errors.append("DNF metadata corruption on CentOS 8 must use 'cleanup_and_retry'.")
        ...
        if isinstance(cleanup, list):
            ...
            if "dnf clean all" not in cleanup_set:
                errors.append(
                    "CentOS 8 metadata corruption cleanup must include 'dnf clean all'."
                )
        else:
            errors.append("CentOS 8 metadata corruption requires 'cleanup' to be a list.")

        # retry must include dnf makecache
        if not any("dnf makecache" in c for c in retry_list):
            errors.append("CentOS 8 metadata corruption retry must include 'dnf makecache'.")
        ...
        if pkg_from_cmd:
            ...
            if not any(expected_install in c for c in retry_list):
                errors.append(
                    f"CentOS 8 metadata corruption retry must include '{expected_install}' "
    ...
    if "rpmdb open failed" in stderr:
        if action != "cleanup_and_retry":
            errors.append("rpmdb corruption on CentOS 8 must use 'cleanup_and_retry'.")
        ...
        if isinstance(cleanup, list):
            ...
            if "rm -f /var/lib/rpm/.rpm.lock" not in cleanup_set:
                errors.append(
                    "CentOS 8 rpmdb corruption cleanup must include 'rm -f /var/lib/rpm/.rpm.lock'."
                )
        else:
            errors.append("CentOS 8 rpmdb corruption requires 'cleanup' to be a list.")

        # retry must include rpm --rebuilddb
        if not any("rpm --rebuilddb" in c for c in retry_list):
            errors.append("CentOS 8 rpmdb corruption retry must include 'rpm --rebuilddb'.")
        ...
        if pkg_from_cmd:
            ...
            if not any(expected_install in c for c in retry_list):
                errors.append(
                    f"CentOS 8 rpmdb corruption retry must include '{expected_install}' "
                    "when a package is present."
                )
    ...
    if "already installed" in stderr or "Nothing to do" in stderr:
        if action != "fallback":
            errors.append("CentOS 8 idempotent install/update must use 'fallback'.")
    ...
    if exit_status == 127 and command.startswith("show "):
        if action != "fallback":
            errors.append("CentOS 8 must use 'fallback' for Cisco-style 'show' commands.")
