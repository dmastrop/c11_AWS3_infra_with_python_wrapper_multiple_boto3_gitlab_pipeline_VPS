...
try:
    ...
    print(f"Launched HTTP EC2 instance with ID: {instance_id}")
    sys.stdout.flush()
except Exception as e:
    print(f"Error launching HTTP EC2 instance: {e}")
    sys.stdout.flush()
    exit(1)
...
#def wait_for_instance_running(instance_id, ec2_client):
    while True:
        try:
            ...
            print(f"HTTP Instance status: {instance_status}")
            sys.stdout.flush()
...
#             time.sleep(10)
            ...
            if instance_status['InstanceStatuses']:
                if (instance_status['InstanceStatuses'][0]['InstanceState']['Name'] == 'running' and
                    instance_status['InstanceStatuses'][0]['SystemStatus']['Status'] == 'ok' and
                    instance_status['InstanceStatuses'][0]['InstanceStatus']['Status'] == 'ok'):
                    print(f"HTTP Instance {instance_id} is now running and has passed status checks.")
                    sys.stdout.flush()
                    instance_ready_event.set()  # Signal that the instance is ready for multi-threading
                    break
                else:
                    print(f"Waiting for HTTP instance {instance_id} to be in running state and pass status checks...")
                    sys.stdout.flush()
                    time.sleep(10)
            else:
                print(f"No status available for HTTP instance {instance_id}. Waiting...")
                sys.stdout.flush()
                time.sleep(10)
...
# added instance_id as argument to this function for multi-threading. This is another scope issue
    ...
    commands = [
        ...
        f"echo 'while true; do wget -q -O- http://{beanstalk_url}; done' > stress_test.sh",
