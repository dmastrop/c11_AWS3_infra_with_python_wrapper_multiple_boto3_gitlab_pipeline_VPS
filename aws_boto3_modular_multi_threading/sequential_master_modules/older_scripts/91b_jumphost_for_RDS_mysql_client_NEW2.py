...
# The error you're encountering indicates that an instance profile can only contain one role, and this limit cannot be increased [1](https://docs.aws.amazon.com/IAM/latest/APIReference/API_AddRoleToInstanceProfile.html) [2](https://github.com/hashicorp/terraform/issues/11575). To resolve this issue, you need to remove the existing role from the instance profile before adding the new role.
...
while True:
    try:
        ...
        print(f"Current status: {ec2_instance_status}. Instance status check: {instance_status_check}. System status check: {system_status_check}. Waiting for EC2 instance to be in running state and status checks to pass...")
        time.sleep(10)
    except ClientError as e:
