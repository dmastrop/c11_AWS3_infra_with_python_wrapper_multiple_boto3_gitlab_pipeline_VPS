...
# ---------------------------------------------------------------------
# TEST 10C — cleanup stderr → cleanup OK → retry succeeds → install_success
# ---------------------------------------------------------------------
def test_ai_hook_cornercase_cleanup_stderr_retry_success(monkeypatch):
    """
    Corner Case 10C:
    Cleanup commands produce stderr but exit=0 (allowed).
    ...
    # cleanup1: exit=0, stderr="warn1"
    # cleanup2: exit=0, stderr="warn2"
    # retry1:   exit=0, stdout="fixed", stderr=""
    # --------------------------------------------------------
    script = [
        ("", "", 1),   # attempt 1
        ("", "", 1),   # attempt 2
        ...
        ("cleanup1 ok", "cleanup warning 1", 0),
        ("cleanup2 ok", "cleanup warning 2", 0),

        ("retry fixed", "", 0),   # retry succeeds
    ]
    ...
    # --------------------------------------------------------
    def fake_send(self, context):
        print("FAKE_AI_HOOK: cleanup stderr → retry success")
        return {
            "action": "cleanup_and_retry",
            "cleanup": [
                "echo CLEANUP1",
    ...
    # cleanup + retry commands recorded
    cmds = registry["ai_metadata"]["ai_commands"]
    assert len(cmds) == 3
    assert cmds[0] == "echo CLEANUP1"
    assert cmds[1] == "echo CLEANUP2"
    assert cmds[2] == "echo RETRY_FIXED"
...
# ---------------------------------------------------------------------
def test_ai_hook_cornercase_retry_stderr_harmless(monkeypatch):
    """
    Corner Case 10D:
    Cleanup succeeds.
    Retry command produces harmless stderr but exit=0.
    ...
    # --------------------------------------------------------
    def fake_send(self, context):
        print("FAKE_AI_HOOK: retry stderr (harmless)")
        return {
            "action": "cleanup_and_retry",
            "cleanup": [
                "echo CLEANUP1",
